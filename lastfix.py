...
class Database:
    ...
    def deduct_balance(self, user_id, amount):
        ...
        result = self.db.users.update_one(
            {"user_id": user_id, "balance": {"$gte": float(amount)}},
    ...
    def get_plan_by_id(self, plan_row_id):
        ...
        if plan:
            return {
                ...
                "is_manual": plan.get("is_manual", False)
            }
        return None
    ...
    def add_product(self, category, name, product_id, plan, price, android_id, is_manual=False):
    ...
    def add_manual_keys(self, product_id, plan, keys):
        for k in keys:
            self.db.manual_keys.insert_one({"product_id": str(product_id), "plan": str(plan), "key": k, "used": False})
        return len(keys)

    def get_manual_key(self, product_id, plan):
        doc = self.db.manual_keys.find_one_and_update(
            {"product_id": str(product_id), "plan": str(plan), "used": False},
            {"$set": {"used": True}}
        )
        if doc:
            return doc["key"]
    ...
    def get_manual_stock(self, product_id, plan):
        return self.db.manual_keys.count_documents({"product_id": str(product_id), "plan": str(plan), "used": False})

    def delete_product_by_id(self, item_id):
        self.db.products.delete_one({"id": int(item_id)})
    ...
    def set_product_voice(self, product_id, chat_id, msg_id):
        if chat_id is None:
            self.db.products.update_many({"product_id": product_id}, {"$unset": {"voice_file_id": "", "voice_chat_id": "", "voice_msg_id": ""}})
        else:
            self.db.products.update_many({"product_id": product_id}, {"$unset": {"voice_file_id": ""}, "$set": {"voice_chat_id": str(chat_id), "voice_msg_id": str(msg_id)}})
...
def fetch_license_key(product_id, duration, android_id=ANDROID_ID):
    ...
    try:
        ...
        response = cureq.post(API_ENDPOINT, data=payload, headers=headers, impersonate="chrome120", timeout=15)
...
def get_main_menu_keyboard(is_admin=False):
    ...
    if db.get_setting("trial_mode") == "1":
        btn_name = db.get_setting("trial_btnname") or "GET FREE TRIAL APK"
        btn_emoji = db.get_setting("trial_btnemoji") or ""
        btn_color = db.get_setting("trial_btncolor") or "danger"
        if btn_emoji and btn_emoji.isdigit():
            icon = btn_emoji
            text = btn_name
        else:
            icon = ""
            text = f"{btn_emoji} {btn_name}" if btn_emoji else btn_name
        rows.insert(0, [CB(text, style=btn_color, icon=icon, callback_data="menu_trial")])
    if is_admin:
...
def get_admin_panel_keyboard():
    return InlineKeyboardMarkup([
        ...
        [CB("Add API Product", style="danger", icon=get_button_emoji("add"), callback_data="admin_addproduct"),
         CB("Add Manual Product", style="danger", icon=get_button_emoji("add"), callback_data="admin_addmanualprod")],
        [CB("Add Manual Keys", style="danger", icon=get_button_emoji("add"), callback_data="admin_addkeys_list")],
        ...
        [CB("Manage Resellers", style="danger", icon=get_button_emoji("star"), callback_data="admin_resellers"),
         CB("Analytics", style="danger", icon=get_button_emoji("stats"), callback_data="admin_stats")],
        ...
        [CB("Trending Product", style="danger", icon=get_button_emoji("star"), callback_data="admin_trending_menu"),
        ...
        [CB("Manage Free Trial", style="danger", icon=get_button_emoji("star"), callback_data="admin_trial_menu")],
...
async def process_purchase_with_android_id(update: Update, context: ContextTypes.DEFAULT_TYPE, product_data, android_id):
    ...
    try:
        ...
        print("Admin Notify Error (Bal):", e)
...
"""
    ...
    await processing_msg.edit_text(text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")

    product_doc = db.db.products.find_one({"product_id": product_id})
    if product_doc:
        voice_id = product_doc.get("voice_file_id")
        v_chat_id = product_doc.get("voice_chat_id")
        v_msg_id = product_doc.get("voice_msg_id")
        link = product_doc.get("product_link")
        logger.info(f"Android ID Purchase Delivery: v_chat_id={v_chat_id}, v_msg_id={v_msg_id}, link={link}")
        if v_chat_id and v_msg_id:
            try: await context.bot.copy_message(chat_id=update.effective_chat.id, from_chat_id=int(v_chat_id), message_id=int(v_msg_id))
            except Exception as e: logger.error(f"Error copying product voice: {e}")
        elif voice_id:
...
async def broadcast_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    flags = ["awaiting_broadcast", "awaiting_addproduct", "awaiting_addbalance", "awaiting_removebalance",
             ...
             "awaiting_trending_product", "awaiting_trending_voice"]
...
async def handle_admin_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    addkeys_id = context.user_data.get("awaiting_manual_addkeys")
    if addkeys_id:
        ...
        if plan_info:
            added = db.add_manual_keys(plan_info["product_id"], plan_info["plan"], keys)
            context.user_data.pop("awaiting_manual_addkeys", None)
            
            keyboard = [
                [CB("Add More Keys", style="primary", icon=get_button_emoji("add"), callback_data=encode_cb("admin_manual_addkeys", addkeys_id))],
    ...
    if step:
        ...
        if step == 1:
            ...
            import random
            import string
            prod_id = "MANUAL_" + "".join(random.choices(string.ascii_uppercase + string.digits, k=6))
            ...
            # Save keys
            added = db.add_manual_keys(prod_id, data["plan"], keys)
            
            context.user_data.pop("awaiting_manual_step", None)
            
            keyboard = [
                [CB(f"Add Another Plan to {data['name']}", style="success", icon="", callback_data=encode_cb("admin_add_plan_again", data["category"], data["name"]))],
                [CB("Back to Admin Panel", style="primary", icon="", callback_data="admin_panel")]
            ]
    ...
    if trial_step:
        ...
        if trial_step == "apk":
            ...
            if not match and update.message.entities:
                for ent in update.message.entities:
                    if ent.type == "custom_emoji":
                        match = type('obj', (object,), {'group': lambda self, n: ent.custom_emoji_id})()
                        break
            if match:
...
async def _safe_edit_text(msg, text, reply_markup, parse_mode):
    try:
        ...
        if "Message is not modified" in str(e):
            pass
        elif "Document_invalid" in str(e) or "entity" in str(e).lower():
            import re
    ...
    if is_media:
        ...
        try:
            ...
            if "entity" in str(e).lower() or "parse" in str(e).lower():
                await query.get_bot().send_message(chat_id=msg.chat_id, text=text, reply_markup=reply_markup, parse_mode=None)
    else:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message:
        ...
        if db.get_setting("trending_enabled") == "1":
            chat_1 = db.get_setting("trending_chat_id")
            msg_1 = db.get_setting("trending_msg_id")
            chat_2 = db.get_setting("trending_voice_chat_id")
            msg_2 = db.get_setting("trending_voice_msg_id")
            
            if chat_1 and msg_1:
                try:
                    sent_msg = await context.bot.copy_message(
                        chat_id=update.effective_chat.id,
                        from_chat_id=int(chat_1),
                        message_id=int(msg_1)
                    )
                    try:
                        await context.bot.pin_chat_message(chat_id=update.effective_chat.id, message_id=sent_msg.message_id, disable_notification=True)
                    except Exception as pin_e:
                        logger.error(f"Failed to pin trending message: {pin_e}")
                except Exception as e:
                    logger.error(f"Error copying trending message 1: {e}")

            if chat_2 and msg_2:
                try:
                    await context.bot.copy_message(
                        chat_id=update.effective_chat.id,
                        from_chat_id=int(chat_2),
                        message_id=int(msg_2)
                    )
                except Exception as e:
                    logger.error(f"Error copying trending message 2: {e}")
...
"""
            ...
            if product_doc:
                ...
                link = product_doc.get("product_link")
                if v_chat_id and v_msg_id:
                    try: await context.bot.copy_message(chat_id=query.message.chat_id, from_chat_id=int(v_chat_id), message_id=int(v_msg_id))
...
"""
        ...
        def get_btn_text_icon(name_key, emoji_key, default_name, default_emoji):
            name = db.get_setting(name_key) or default_name
            emoji = db.get_setting(emoji_key) or default_emoji
            if emoji and emoji.isdigit():
                return name, emoji
            else:
                return f"{emoji} {name}" if emoji else name, ""

        key_n, key_e = get_btn_text_icon("trial_btn_key_name", "trial_btn_key_emoji", "Get Trial Key", get_button_emoji("star"))
        apk_n, apk_e = get_btn_text_icon("trial_btn_apk_name", "trial_btn_apk_emoji", "Get APK", get_button_emoji("download"))
        setup_n, setup_e = get_btn_text_icon("trial_btn_setup_name", "trial_btn_setup_emoji", "How to Setup", get_button_emoji("tutorial"))
        feedback_n, feedback_e = get_btn_text_icon("trial_btn_feedback_name", "trial_btn_feedback_emoji", "Send Feedback", get_button_emoji("support"))

        apk_url = db.get_setting("trial_apk") or "https://t.me/yourchannel"
        setup_url = db.get_setting("trial_setuplink") or "https://t.me/yourchannel"
        feedback_url = db.get_setting("trial_feedbacklink") or "https://t.me/yourchannel"
        
        kb = [
            [CB(key_n, style=db.get_setting("trial_btn_key_color") or "primary", icon=key_e, callback_data="trial_get_key")],
            [CB(apk_n, style=db.get_setting("trial_btn_apk_color") or "success", icon=apk_e, url=apk_url)],
            [CB(setup_n, style=db.get_setting("trial_btn_setup_color") or "primary", icon=setup_e, url=setup_url)],
        ...
        if menu_msg_chat_id and menu_msg_id:
            try:
                await query.message.delete()
            except:
                pass
            try:
                await context.bot.copy_message(chat_id=query.message.chat_id, from_chat_id=menu_msg_chat_id, message_id=int(menu_msg_id), reply_markup=InlineKeyboardMarkup(kb))
                return
            except Exception as e:
                pass
                
        await safe_edit(query, caption, InlineKeyboardMarkup(kb), parse_mode="HTML")

    elif data == "trial_get_key":
        await safe_answer(query, )
        key_msg_chat_id = db.get_setting("trial_key_msg_chat_id")
        key_msg_id = db.get_setting("trial_key_msg_id")
        
        sent = False
        if key_msg_chat_id and key_msg_id:
            try:
                await context.bot.copy_message(chat_id=query.message.chat_id, from_chat_id=key_msg_chat_id, message_id=int(key_msg_id))
                sent = True
            except Exception as e:
                pass
        ...
        trial_msg_id = db.get_setting("trial_msg_id")
        trial_apk = db.get_setting("trial_apk") or ""
        
        await safe_answer(query, "Sending APK...")
        
        sent = False
        if trial_chat_id and trial_msg_id:
            try:
                await context.bot.copy_message(chat_id=query.message.chat_id, from_chat_id=int(trial_chat_id), message_id=int(trial_msg_id))
                sent = True
            except Exception:
                pass
    ...
    elif data == "trial_get_setup":
        trial_setup = db.get_setting("trial_setup") or "1. Download the APK\n2. Install it\n3. Use the key to login"
        trial_voice = db.get_setting("trial_voice_id")
        
        await safe_answer(query, "Sending Setup Guide...")
        
        if trial_voice:
            try:
                await context.bot.send_voice(chat_id=query.message.chat_id, voice=trial_voice)
            except Exception:
                pass
        ...
        if data == "admin_panel":
            ...
            keyboard = [
                ...
                [CB("Feedback Link", style="danger", icon=get_button_emoji("support"), callback_data="admin_trial_set_feedbacklink"),
                 CB("Set Trial Key", style="danger", icon=get_button_emoji("star"), callback_data="admin_trial_set_key")],
                [CB("Back to Admin", style="primary", icon=get_button_emoji("back"), callback_data="admin_panel")]
            ]
            await safe_edit(query, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")

        elif data == "admin_trial_toggle":
            current = db.get_setting("trial_mode") or "0"
            ...
            c_feedback = db.get_setting("trial_btn_feedback_color") or "danger"
            keyboard = [
                [CB("Key Name", style="danger", icon="", callback_data="admin_trial_set_key_name"),
                 CB("Key Emoji", style="danger", icon="", callback_data="admin_trial_set_key_emoji"),
                 CB(f"Color: {c_key}", style=c_key, icon="", callback_data="admin_trial_color_key")],
                 
                [CB("APK Name", style="danger", icon="", callback_data="admin_trial_set_apk_name"),
                 CB("APK Emoji", style="danger", icon="", callback_data="admin_trial_set_apk_emoji"),
                 CB(f"Color: {c_apk}", style=c_apk, icon="", callback_data="admin_trial_color_apk")],
                 
                [CB("Setup Name", style="danger", icon="", callback_data="admin_trial_set_setup_name"),
                 CB("Setup Emoji", style="danger", icon="", callback_data="admin_trial_set_setup_emoji"),
                 CB(f"Color: {c_setup}", style=c_setup, icon="", callback_data="admin_trial_color_setup")],
                 
                [CB("Feedback Name", style="danger", icon="", callback_data="admin_trial_set_feedback_name"),
                 CB("Feedback Emoji", style="danger", icon="", callback_data="admin_trial_set_feedback_emoji"),
                 CB(f"Color: {c_feedback}", style=c_feedback, icon="", callback_data="admin_trial_color_feedback")],
                 
                [CB("Back", style="primary", icon=get_button_emoji("back"), callback_data="admin_trial_menu")]
            ]
            await safe_edit(query, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
        ...
        elif data.startswith("admin_trial_color_"):
            btn = data.split("admin_trial_color_")[1]
            styles = ["primary", "success", "danger"]
            current = db.get_setting(f"trial_btn_{btn}_color") or ("success" if btn=="apk" else "primary")
            try:
                next_style = styles[(styles.index(current) + 1) % len(styles)]
            except:
                next_style = "primary"
            ...
            for p in products:
                ...
                keyboard.append([CB(label, style="primary", icon=get_button_emoji("star"), callback_data=encode_cb("admin_addkeys_sel", p['id']))])
            keyboard.append([CB("Back", style="danger", icon=get_button_emoji("back"), callback_data="admin_panel")])
            await safe_edit(query, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")

        elif data.startswith("admin_addkeys_sel" + SEP) or data.startswith("admin_manual_dash" + SEP):
            parts = decode_cb(data)
            plan_id = parts[1]
            ...
            unused_count = db.db.manual_keys.count_documents({"product_id": pid, "plan": p_plan, "used": False})
            ...
            keyboard = [
                ...
                [CB("Manage Active Keys", style="primary", icon="", callback_data=encode_cb("admin_manual_listkeys", plan_id))],
                [CB("View Used Keys", style="primary", icon="", callback_data=encode_cb("admin_manual_usedkeys", plan_id))],
                [CB("Back to List", style="danger", icon=get_button_emoji("back"), callback_data="admin_addkeys_list")]
            ]
            if query.message:
                await safe_edit(query, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
            else:
        ...
        elif data.startswith("admin_manual_listkeys" + SEP):
            parts = decode_cb(data)
            plan_id = parts[1]
            plan_info = db.get_plan_by_id(plan_id)
        ...
        elif data.startswith("admin_manual_usedkeys" + SEP):
            parts = decode_cb(data)
            plan_id = parts[1]
            plan_info = db.get_plan_by_id(plan_id)
            if not plan_info: return
            ...
            # Fetch up to 30 used keys
            keys_cursor = db.db.manual_keys.find({"product_id": pid, "plan": p_plan, "used": True}).sort("_id", -1).limit(30)
            keys_list = list(keys_cursor)
            ...
            keyboard = [[CB("Back to Dashboard", style="primary", icon=get_button_emoji("back"), callback_data=encode_cb("admin_manual_dash", plan_id))]]
            await safe_edit(query, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")

        elif data.startswith("admin_manual_delkey" + SEP):
            parts = decode_cb(data)
            key_oid = parts[1]
            plan_id = parts[2]
            ...
            try:
                key_data = db.db.manual_keys.find_one({"_id": ObjectId(key_oid)})
                if key_data:
            ...
            if plan_info:
                ...
                await safe_edit(query, text="Error loading plan.")
                
        elif data.startswith("admin_manual_addkeys" + SEP):
            parts = decode_cb(data)
            plan_id = parts[1]
            plan_info = db.get_plan_by_id(plan_id)
            if not plan_info:
            ...
            text = (
                ...
                f"Please send the keys now:\n"
                f"(Send /cancel to abort)"
            )
            keyboard = [[CB("Back", style="danger", icon=get_button_emoji("back"), callback_data=encode_cb("admin_manual_dash", plan_id))]]
            if query.message:
                await safe_edit(query, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
            else:
                await query.message.reply_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")

        elif data.startswith("admin_add_plan_again" + SEP):
            parts = decode_cb(data)
            category = parts[1]
            name = parts[2]
            ...
            v_chat_id = prod.get("voice_chat_id") if prod else None
            v_msg_id = prod.get("voice_msg_id") if prod else None
            voice_id = prod.get("voice_file_id") if prod else None
            
            if v_chat_id and v_msg_id:
                await safe_answer(query, "Sending delivery message...")
                await context.bot.copy_message(chat_id=user_id, from_chat_id=int(v_chat_id), message_id=int(v_msg_id))
            elif voice_id:
            ...
            product_id = context.user_data.get("vl_product_id")
            if not product_id: return
            db.set_product_voice(product_id, None, None)
            ...
            keyboard = [
                [CB(f"Status: {status_text}", style="primary", icon="", callback_data="admin_toggle_trending")],
                [CB("Set Product (Photo/Video/Text)", style="danger", icon=get_button_emoji("star"), callback_data="admin_set_trending")],
                [CB("Set Voice (Separately)", style="danger", icon=get_button_emoji("star"), callback_data="admin_set_trending_voice")],
                [CB("Clear Product & Voice", style="danger", icon=get_button_emoji("clear"), callback_data="admin_clear_trending")],
                [CB("Back to Admin", style="danger", icon=get_button_emoji("back"), callback_data="back_to_menu")]
            ]
            await safe_edit(query, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
            
        elif data == "admin_toggle_trending":
            status = db.get_setting("trending_enabled")
        ...
        elif data == "admin_clear_trending":
            db.delete_setting("trending_chat_id")
            db.delete_setting("trending_msg_id")
    ...
    """
            ...
            key = decode_cb(data)[1]
            db.set_setting(key, "") 
            await safe_answer(query, f"Reset {key} to default!", show_alert=True)
            ...
            await safe_answer(query, "Unknown action.", show_alert=True)
